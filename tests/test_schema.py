...
class CommittedDataTest(unittest.TestCase):
    def test_every_trial_validates(self):
        self.assertEqual(len(TRIALS), 218)
        for f in TRIALS:
...
class ExportsInSyncTest(unittest.TestCase):
    ...
    def test_docs_cover_every_field(self):
        ...
        self.assertEqual(len(FIELD_DOCS), 47)  # 35 v1 fields + faers_summary + 3 exclusivity fields + 4 v3 results fields + drug_characterization (cycle 23) + overall_status/results_status/why_stopped (cycle 25)
...
class CsvTest(unittest.TestCase):
    def test_flattened_tables_match_json(self):
        ...
        self.assertEqual(len(trials), 218)
        self.assertEqual(len(sources), 218 * 47)
